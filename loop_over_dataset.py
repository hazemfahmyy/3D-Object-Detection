...
model = "fpn_resnet"
sequence = "1"
results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results/' + 'resnet' + '/results_sequence_' + sequence + '_' + 'resnet')
...
exec_visualization = ['pcl_from_rangeimage', 'bev_from_pcl', 'detect_objects', 'validate_object_labels','measure_detection_performance', 'show_detection_performance'] # options are 'show_range_image', 'show_bev', 'show_pcl', 'show_labels_in_image', 'show_objects_and_labels_in_bev', 'show_objects_in_bev_labels_in_camera', 'show_tracks', 'show_detection_performance', 'make_tracking_movie'
...
while True:
    try:
        ...
        if 'show_range_image' in exec_list:
            ...
            deg_90 = int(img_range.shape[1]/4)
            center = int(img_range.shape[1]/2)
            img_range_cropped = img_range[:, (center - deg_90):(center + deg_90)]
            
            img_range = img_range.astype(np.uint8)
            img_range_cropped = img_range_cropped.astype(np.uint8)
            
            cv2.imshow('range_image', img_range)
            cv2.imwrite('./results/range_image_frame' + str(cnt_frame) + '.png', img_range)
            cv2.waitKey(vis_pause_time)
            
            cv2.imshow('range_image', img_range_cropped)
            cv2.imwrite('./results/range_image_cropped_frame' + str(cnt_frame) + '.png', img_range_cropped)
            cv2.waitKey(vis_pause_time)
            

        if 'show_pcl' in exec_list:
            pcl.show_pcl(lidar_pcl)
            #cv2.imwrite('./results/pcl_frame' + str(cnt_frame) + '.png', pcl)
